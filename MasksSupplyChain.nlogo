...
to interToProds
  set reqs_prod (reqs_prod + reqs_inter)
end
...
to prodsToInter
  ...
  set inv_producer (inv_producer + curWeekDel)

  if inv_producer >= weeklyMasksRequirement [
    set inv_inter_receive (inv_inter_receive + weeklyMasksRequirement)

    set inv_producer (inv_producer - weeklyMasksRequirement)

    set reqs_prod (reqs_prod - weeklyMasksRequirement)
  ]
end
...
to create-lnks
  ask consumer 0 [
    create-link-with inter 5
  ]
  ask inter 5 [
    create-link-with producer 1
  ]
  ask inter 5 [
    create-link-with producer 2
  ]
  ask inter 5 [
    create-link-with producer 3
  ]
  ask inter 5 [
    create-link-with producer 4
  ]
end
